...
- iconText `#fff` overlay: upstream draws white text over a `blank` (fully transparent) icon, which would be invisible on the default light background; we instead draw a `mainBkg`-filled box with `textColor` text (readable, theme-adaptive). Kept our pragmatic rendering — matching upstream's literal white-on-transparent would be a regression, not a parity gain. Niche `(("text"))` form only. *(Superseded by the P2 pass below — `blank` is actually an opaque `#087ebf` box.)*

### P2 — colors/icons/shapes parity (icon box + glyph fidelity)
Symptom: services looked nothing like upstream — bare single-colour line glyphs in `textColor`, no fill box; weak icon shapes. Root cause: our `_iconRef` pointed at the generic 24×24 Material `icon:` pack and `renderIcon` could only emit single-colour *filled* paths, so it had no way to reproduce upstream's `mermaid-architecture` glyphs (a solid `#087ebf` box overlaid with white *stroked* line art) — see `architectureIcons.ts` `wrapIcon`.
- `icon_registry.dart`: rewrote `renderIcon` to parse every drawable SVG element (`path`/`line`/`rect`/`circle`/`ellipse`) and honour each element's inline `style`/attrs (`fill`, `stroke`, `stroke-width`, `none`). Elements with no explicit paint still default to a solid fill in the passed `color`, so the existing single-colour `icon:` glyphs (flowchart/mindmap) and the `icons_test` "every shape has a fill" assertion are unchanged. Added a `_parseColor` helper and stroke-width scaling.
- `icon_registry.dart`: registered a new `mermaid-architecture` pack (80×80) with the verbatim upstream bodies for `database`/`server`/`disk`/`internet`/`cloud`/`blank` — each prefixed with the `#087ebf` background rect. Registered alongside the builtin pack in `ensureBuiltinIconPacks`.
- `architecture.dart`: `_iconRef` now maps the arch names to `mermaid-architecture:*` (unknown unqualified names fall back to the opaque `blank` box, mirroring upstream's `fallbackPrefix`; explicit `prefix:name` refs still honoured). Service-with-icon no longer draws a separate border (the glyph paints its own box). `iconText` now renders the `mermaid-architecture:blank` opaque teal box with **white** centered text (upstream `.node-icon-text > div { color:#fff }`).
- Confirmed default-theme constants unchanged: edge width 3 = `archEdgeWidth`, group border width 2 + `dash [8,8]` = `archGroupBorderWidth`/`stroke-dasharray:8`, group/plain-service border = `primaryBorderColor`, edges/arrows = `lineColor`, font 16 = config `fontSize`. All still read from `MermaidTheme`. `dart analyze` clean; `icons_test`/`new_types_test` green.
