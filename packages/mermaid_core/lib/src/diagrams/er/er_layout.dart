...
// [ErConfig.entityPadding] resolves).
...
class ErConfig {
  ...
  factory ErConfig.fromSource(String source) {
    final values = resolveDiagramConfig(source, 'er');
    ...
    return ErConfig(
      entityPadding: value('entityPadding', 15),
      diagramPadding: value('diagramPadding', 20),
      minEntityWidth: value('minEntityWidth', 100),
      minEntityHeight: value('minEntityHeight', 75),
      nodeSpacing: value('nodeSpacing', 140),
      rankSpacing: value('rankSpacing', 80),
      titleTopMargin: value('titleTopMargin', 25),
    );
  }
}
...
class _ErLayout {
  ...
  RenderScene run() {
    ...
    for (final b in boxes.values) {
      g.addNode(dagre.DagreNode(b.entity.id, width: b.width, height: b.height));
    }
    ...
    for (var i = 0; i < diagram.relationships.length; i++) {
      ...
      if (labelSize != null) {
        final c = r.from == r.to
            ? Point(
                points[1].x + 8 + labelSize.width / 2,
  ...
  _EntityBox _measureEntity(ErEntity e) {
    ...
    if (cells.isEmpty) {
      ...
      var width = headerSize.width + config.diagramPadding * 2;
      if (width < config.minEntityWidth) width = config.minEntityWidth;
      final height = math.max(
        headerSize.height + config.diagramPadding * 1.5 * 2,
        config.minEntityHeight,
      );
    ...
    for (final row in cells) {
      ...
      for (var c = 0; c < row.length; c++) {
        ...
        colWidths[c] = math.max(
          colWidths[c],
          size.width + config.diagramPadding,
        );
    ...
    var width = math.max(
      headerSize.width + config.diagramPadding * 2,
      colWidths.fold(0.0, (a, b) => a + b),
    );
    width = math.max(width, config.minEntityWidth);
    ...
    if (height < config.minEntityHeight) {
      ...
      final extra = (config.minEntityHeight - height) / rowHeights.length;
      for (var i = 0; i < rowHeights.length; i++) {
        rowHeights[i] += extra;
      }
      height = config.minEntityHeight;
    }
  ...
  SceneGroup _buildEntity(_EntityBox b) {
    ...
    final labelBounds = cells.isEmpty
        ? Rect.fromLTWH(
            rect.left,
            rect.center.y - headerSize.height / 2,
            rect.width,
            headerSize.height,
          )
        : Rect.fromLTWH(
